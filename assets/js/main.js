...
var Core = function () {
    ...
    var runSideMenu = function () {
        ...
        var toggleLeftSideMenu = function () {
			...
			$("header.navbar").css("width",ori_nav_width);
        }
		...
		var sidebarCheck = function() {
			if ($(window).width() < 1080) {	$('body').addClass('mobile-view sidebar-hidden').removeClass('sidebar-ltr sidebar-rtl'); }
		...
		$(document).ready(function(){
			sidebarCheck();
			ori_nav_width = $("header.navbar").css("width");
		});
	...
	function runBasic(){
		$('#logoutBtnForm').click(function(){
			$('#logoutForm').find('[name="logoutBtn"]').click();
		});
	}
